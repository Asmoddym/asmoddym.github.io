...
class Norska_Integration {
	...
	function send() {
 	...
 	function send_subject() {
		if (strstr($this->run, "FAILURE")) {
			...
			if (strstr($this->run, "Fatal error")) {
				$subject = "FATAL";
			} elseif (strstr($this->run, "Parse error")) {
				$subject = "PARSE";
			} else { 
				$subject = "SUCCESS";
			}
	...
	function hook($method, $parameters = null) {
		...
		if ($this->hook_object instanceof Norska_Hooks and method_exists($class_hook, $method)) {
