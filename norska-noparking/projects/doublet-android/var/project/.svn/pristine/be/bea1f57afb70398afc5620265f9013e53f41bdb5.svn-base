...
class PolygonAnnotationPicker(internal val activity: AppCompatActivity) {
	...
	private fun updateView() {
		if (_data != null) {
			_view.layout_polygon_annotation_width.text.replace(0, _view.layout_polygon_annotation_width.text.length, when (_data?.width) {
				null -> ""
				else -> _data?.width.toString()
			})
			_view.layout_polygon_annotation_height.text.replace(0, _view.layout_polygon_annotation_height.text.length, when (_data?.height) {
				null -> ""
				else -> _data?.height.toString()
			})
			_view.layout_polygon_annotation_surface.text.replace(0, _view.layout_polygon_annotation_surface.text.length, _data?.surface.toString())
			_view.layout_polygon_annotation_comment.text.replace(0, _view.layout_polygon_annotation_comment.text.length, _data?.comment)
			_view.layout_polygon_annotation_surface.text.replace(0, _view.layout_polygon_annotation_surface.text.length, _data?.surface.toString())
		}
		...
		if (_on_save != null) {
			_view.layout_polygon_annotation_save.setOnClickListener {
				if (_on_save != null) {
					_on_save?.invoke(
							_id,
							when (_view.layout_polygon_annotation_width.text.toString().isEmpty()) {
								true -> null
								else -> _view.layout_polygon_annotation_width.text.toString().toFloat()
							},
							when (_view.layout_polygon_annotation_height.text.toString().isEmpty()) {
								true -> null
								else -> _view.layout_polygon_annotation_height.text.toString().toFloat()
							},
							when (_view.layout_polygon_annotation_surface.text.toString().isEmpty()) {
								true -> 0f
								else -> _view.layout_polygon_annotation_surface.text.toString().toFloat()
							},
