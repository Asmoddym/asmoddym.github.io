  ...
  public LineDataDao_Impl(RoomDatabase __db) {
    ...
    this.__insertionAdapterOfLine = new EntityInsertionAdapter<Line>(__db) {
      ...
      public String createQuery() {
        return "INSERT OR REPLACE INTO `lines`(`id`,`polygon_id`,`from_x`,`from_y`,`to_x`,`to_y`,`color`) VALUES (?,?,?,?,?,?,?)";
      }
      ...
      public void bind(SupportSQLiteStatement stmt, Line value) {
        ...
        stmt.bindDouble(3, value.getFrom_x());
        stmt.bindDouble(4, value.getFrom_y());
        stmt.bindDouble(5, value.getTo_x());
        stmt.bindDouble(6, value.getTo_y());
    ...
    this.__updateAdapterOfLine = new EntityDeletionOrUpdateAdapter<Line>(__db) {
      @Override
      public String createQuery() {
        return "UPDATE OR REPLACE `lines` SET `id` = ?,`polygon_id` = ?,`from_x` = ?,`from_y` = ?,`to_x` = ?,`to_y` = ?,`color` = ? WHERE `id` = ?";
      }
