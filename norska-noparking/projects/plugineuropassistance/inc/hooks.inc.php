...
class Norska_Plugineuropassistance_Hooks extends Norska_Opentime_Hooks {
	function install_after () {
		$this->configure_filerights($this->parameters['path']);
		$this->configure_opentime($this->parameters['path'] . "/opentime/");
		$this->configure_opentime($this->parameters['path'] . "/plugins/europassistance/");
	}

	function send_subject_after($subject) {
		return "[nopkg europassistance ".$this->svn->number_revision()."] ".$subject;
	}
