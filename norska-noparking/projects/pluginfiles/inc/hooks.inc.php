...
class Norska_Pluginfiles_Hooks extends Norska_Opentime_Hooks {
	function install_after () {
		$this->configure_filerights($this->parameters['path']);
		$this->configure_opentime($this->parameters['path'] . "/opentime/");
		$this->configure_plugin($this->parameters['path'] . "/plugins/files/");
	}
	...
	function send_subject_after($subject) {
		return "[nopkg pluginfiles ".$this->svn->number_revision()."] ".$subject;
	}
